...
a.shadow.color);e.fillStyle="#fff";e.fillStyle=a.color;e.fillText(a.text,0,0);if(a.fixed){for(var b=e.getImageData(0,0,c.width,c.height),d=c.height-1,g,h=0,k=b.data;0<=d;d--)for(g=c.width-1;0<=g;g--)0!=k[4*(d*c.width+g)+3]&&g>h&&(h=g+1);c.width=h;e.putImageData(b,0,0)}this.thread.push({canvas:c,fixed:a.fixed,index:this.thread.index,speed:.3*Math.pow(c.width,1/3),offset:{x:this.canvas.width,y:this.canvas.font.size*this.thread.index}});return this};f.prototype.clear=function(){this.thread.empty();return this};
